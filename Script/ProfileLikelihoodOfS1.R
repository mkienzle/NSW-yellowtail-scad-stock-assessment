...
local.prob.for.ll.model1 <- function(par, fixeds1, catch, effort, catchability.scaling.factor){
    ...
    M <- par[2]
    selectivity.at.age <- c(fixeds1, par[3], par[4], par[5], par[6], par[7], rep(1, ncol(catch) - 6))
...
local.ll.model1 <- function(par, fixeds1, catch, effort, catchability.scaling.factor){
    ...
    P <- local.prob.for.ll.model1(par, fixeds1, catch, effort, catchability.scaling.factor)
...
bound <- data.frame(lower = c("q" = 1e-2, "M" = 1e-3, "s2" = 1e-2, "s3" = 1e-2, "s4" = 1e-2, "s5" = 1e-2, "s6" = 1e-2),
                    upper = c("q" = 15, "M" = 1, "s2" = 5e-1, "s3" = 1, "s4" = 1, "s5" = 1, "s6" = 1))
...
for(s1 in seq(0, 1e-7, length = 1e3)){
print(paste("Counter=", counter))

result1 <- optim(par = c("q" = 5, "M" = 0.2, "s2" = 0.1, "s3" = 0.3, "s4" = 0.4, "s5" = 0.7, "s6" = 0.9), fn = local.ll.model1, catch = nb.at.age.wgt, effort = effort.mat, catchability.scaling.factor = csf, fixeds1 = s1, method = c("L-BFGS-B"),
       lower = bound[, "lower"] , upper = bound[, "upper"], hessian = TRUE)

my.res[counter, "s1"] <- s1
my.res[counter, "log.lik"] <- result1$value
my.res[counter, c("q", "M","s2","s3","s4","s5","s6")] <- result1$par
counter <- counter +1;
#print(my.res)
}

write.csv(my.res, paste("Results/Data/ProfileLikelihoodOfS1-", format(Sys.time(), "%b%d%Y-%H-%M-%S"), ".csv", sep=""))
