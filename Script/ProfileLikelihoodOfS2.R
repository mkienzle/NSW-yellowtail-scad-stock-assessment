...
local.prob.for.ll.model1 <- function(par, fixeds2, catch, effort, catchability.scaling.factor){
    ...
    M <- par[2]
    selectivity.at.age <- c(par[3], fixeds2, par[4], par[5], par[6], par[7], rep(1, ncol(catch) - 6))
...
local.ll.model1 <- function(par, fixeds2, catch, effort, catchability.scaling.factor){
    ...
    P <- local.prob.for.ll.model1(par, fixeds2, catch, effort, catchability.scaling.factor)
...
for(s2 in seq(1e-2, 1.5e-1, length = 1e3)){
print(paste("Counter=", counter))

result1 <- optim(par = c("q" = 5, "M" = 0.2, "s1" = 0, "s3" = 0.3, "s4" = 0.4, "s5" = 0.7, "s6" = 0.9), fn = local.ll.model1, catch = nb.at.age.wgt, effort = effort.mat, catchability.scaling.factor = csf, fixeds2 = s2, method = c("L-BFGS-B"),
       lower = bound[, "lower"] , upper = bound[, "upper"], hessian = TRUE)

my.res[counter, "s2"] <- s2
my.res[counter, "log.lik"] <- result1$value
my.res[counter, c("q", "M","s1","s3","s4","s5","s6")] <- result1$par
counter <- counter +1;
#print(my.res)
}

write.csv(my.res, paste("Results/Data/ProfileLikelihoodOfS2-", format(Sys.time(), "%b%d%Y-%H-%M-%S"), ".csv", sep=""))
