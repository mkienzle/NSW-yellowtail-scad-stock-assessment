...
local.prob.for.ll.model1 <- function(par, fixeds4, catch, effort, catchability.scaling.factor){
    ...
    M <- par[2]
    selectivity.at.age <- c("s1" = par[3], "s2" = par[4], "s3" = par[5], fixeds4, par[6], par[7], rep(1, ncol(catch) - 6))
...
local.ll.model1 <- function(par, fixeds4, catch, effort, catchability.scaling.factor){
    ...
    P <- local.prob.for.ll.model1(par, fixeds4, catch, effort, catchability.scaling.factor)
...
for(s4 in seq(0.2, 0.7, length = 1e3)){
print(paste("Counter=", counter))

result1 <- optim(par = c("q" = 5, "M" = 0.2, "s1" = 0, "s2" = 0.05, "s3" = 0.17, "s5" = 0.7, "s6" = 0.9), fn = local.ll.model1, catch = nb.at.age.wgt, effort = effort.mat, catchability.scaling.factor = csf, fixeds4 = s4, method = c("L-BFGS-B"),
       lower = bound[, "lower"] , upper = bound[, "upper"], hessian = TRUE)

my.res[counter, "s4"] <- s4
my.res[counter, "log.lik"] <- result1$value
my.res[counter, c("q", "M","s1","s2","s3","s5","s6")] <- result1$par
counter <- counter + 1;
#print(my.res)
}

write.csv(my.res, paste("Results/Data/ProfileLikelihoodOfS4-", format(Sys.time(), "%b%d%Y-%H-%M-%S"), ".csv", sep=""))

with(my.res, plot(s4, log.lik, type = "l"))
