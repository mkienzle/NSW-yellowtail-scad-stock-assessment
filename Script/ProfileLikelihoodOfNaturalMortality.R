local.prob.for.ll.model1.2 <- function(par, fixedM, catch, effort, catchability.scaling.factor){
...
local.ll.model1.2 <- function(par, fixedM, catch, effort, catchability.scaling.factor){
    ...
    P <- local.prob.for.ll.model1.2(par, fixedM, catch, effort, catchability.scaling.factor)
...
lower.bound <- c("q" = 1e-2, "s1" = 0,1e-2,1e-2,1e-2,1e-2,1e-2); upper.bound <- c(15,1e-1,5e-1,1,1,1,1)

my.res <- data.frame("M" = NA, "q" = NA, "s1" = NA, "s2" = NA, "s3" = NA, "s4" = NA, "s5" = NA, "s6" = NA, "log.lik" = NA)
counter <- 1

csf <- 1e-7 # catchability scaling factor

for( M in seq(0.15, 0.30, length = 1e3)){
#for( M in seq(0.15, 0.30, length = 1e1)){
print(paste("Counter=", counter))

result1.2 <- optim(par = c(5,0.1,0.1, 0.3, 0.4,0.7, 0.9), fn = local.ll.model1.2, catch = nb.at.age.wgt, effort = effort.mat, catchability.scaling.factor = csf, fixedM = M, method = c("L-BFGS-B"),
       lower = lower.bound, upper = upper.bound, hessian = TRUE)

my.res[counter, "M"] <- M
my.res[counter, "log.lik"] <- result1.2$value
my.res[counter, c("q","s1","s2","s3","s4","s5","s6")] <- result1.2$par
counter <- counter +1;
print(my.res)
}

write.csv(my.res, paste("Results/Data/ProfileLikelihoodOfNaturalMortality-", format(Sys.time(), "%b%d%Y-%H-%M-%S"), ".csv", sep=""))
