...
local.prob.for.ll.model1 <- function(par, fixeds5, catch, effort, catchability.scaling.factor){
    ...
    M <- par[2]
    selectivity.at.age <- c("s1" = par[3], "s2" = par[4], "s3" = par[5], "s4" = par[6], fixeds5, par[7], rep(1, ncol(catch) - 6))
...
local.ll.model1 <- function(par, fixeds5, catch, effort, catchability.scaling.factor){
    ...
    P <- local.prob.for.ll.model1(par, fixeds5, catch, effort, catchability.scaling.factor)
...
for(s5 in seq(0.5, 1, length = 1e3)){
print(paste("Counter=", counter))

result1 <- optim(par = c("q" = 5, "M" = 0.2, "s1" = 0, "s2" = 0.05, "s3" = 0.17, "s4" = 0.26, "s6" = 0.9), fn = local.ll.model1, catch = nb.at.age.wgt, effort = effort.mat, catchability.scaling.factor = csf, fixeds5 = s5, method = c("L-BFGS-B"),
       lower = bound[, "lower"] , upper = bound[, "upper"], hessian = TRUE)

my.res[counter, "s5"] <- s5
my.res[counter, "log.lik"] <- result1$value
my.res[counter, c("q", "M","s1","s2","s3","s4","s6")] <- result1$par
counter <- counter + 1;
#print(my.res)
}

write.csv(my.res, paste("Results/Data/ProfileLikelihoodOfS5-", format(Sys.time(), "%b%d%Y-%H-%M-%S"), ".csv", sep=""))

with(my.res, plot(s5, log.lik, type = "l"))
